...
var {BooleanField, BoundField, CheckboxFieldRenderer, CheckboxSelectMultiple,
     FileField, Form, RadioFieldRenderer, RadioSelect} = require('newforms')
...
var CheckboxRenderer = CheckboxFieldRenderer.extend({
  render() {
...
var CheckboxInlineRenderer = CheckboxFieldRenderer.extend({
  render() {
    return this.choiceInputs().map(input => {
...
var RadioRenderer = RadioFieldRenderer.extend({
  render() {
    return this.choiceInputs().map(input => {
...
var RadioInlineRenderer = RadioFieldRenderer.extend({
  render() {
    return this.choiceInputs().map(input => {
...
var BootstrapForm = Form.extend({
  constructor: function BootstrapForm(kwargs) {
    ...
    for (var i = 0, l = fieldNames.length; i < l ; i++) {
      ...
      if (field.widget instanceof RadioSelect &&
          field.widget.renderer === RadioFieldRenderer) {
        field.widget.renderer = RadioRenderer
      }
      else if (field.widget instanceof CheckboxSelectMultiple &&
               field.widget.renderer === CheckboxFieldRenderer) {
        field.widget.renderer = CheckboxRenderer
      }
    }
  },
...
BootstrapForm.renderers = {
  CheckboxRenderer
, CheckboxInlineRenderer
, RadioRenderer
, RadioInlineRenderer
}
